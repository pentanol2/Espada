...
def create_final_data_frame():
    ...
    while(x<len(hosts)):
        ...
        final_frame = final_frame.append({'host_id':hosts.iloc[x][0],'title': current_title,'content':currenet_paragraph_concatenation,'average':labels.loc[labels.id == hosts.iloc[x][0]].iloc[0][2],'spamicity':labels.loc[labels.id == hosts.iloc[x][0]].iloc[0][1]},ignore_index=True)
        x +=1
